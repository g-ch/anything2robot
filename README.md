...
This repo assumes a '.stl' 3D model has been acquired by AI text/image to 3D model tools like Meshy. Make sure the model is scaled to a reasonable size, such as 20 cm - 100 cm. You can use ```metamaterial_filling/build/scale_model``` to scale the model.
The command is

```
./scale_model  <path_to_stl_file> <path_to_scaled_stl_file> <scale_factor>
```

The joints should either 1) be acquired by human/animal pose estimation tools; 2) manually added by you in the UI that will be shown when you run the auto design script later. For the first case, the joints must be transformed into the format used in this repo. Check ```auto_design/modules/quadruped_pose_to_pkl.py``` for examples. Note the name of contact point to the ground is treated as a joint should contain string "foot" in its name. Each link should have at least two joints. Axis of a fixed joint is (0,0,0).
...
If you have a pkl file generated by 1), specify the path to the pkl file using ```--joint_pkl_path```. Otherwise, this file will be saved to the provided path when the UI is used. 
